...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab8"], modelMatrix, glm::vec3(1.0f, 0.0f, 0.75f));
	}
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_P))
		{
			...
			//lightDirection = glm::vec3(turn * glm::vec4(lightDirection, 1));
			lightDirection = glm::vec3(lightDirection.x + angleOX, lightDirection.y, lightDirection.z);
