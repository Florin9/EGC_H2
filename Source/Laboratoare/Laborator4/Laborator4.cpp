...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	if (up) {
		btx += deltaTimeSeconds;
		bstep -= 1.1 * deltaTimeSeconds;
		if (btx > 16) {
			up = 0;
		}
	}
	...
	bty = sin(angle);
	modelMatrix = glm::mat4(1);
	modelMatrix *= Transform3D::Translate(2.0f, 1.5f, 2.5f);
	modelMatrix *= Transform3D::Translate(btx, bty, btz);
	modelMatrix *= Transform3D::RotateOZ(bstep);
	RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
...
void Laborator4::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_W)) {
		translateZ -= valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_S)) {
		translateZ += valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_A)) {
		translateX -= valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_D)) {
		translateX += valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_R)) {
		translateY += valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_F)) {
		translateY -= valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_1)) {
		scaleX += valueT * deltaTime;
		scaleY += valueT * deltaTime;
		scaleZ += valueT * deltaTime;
	}
	...
	if (window->KeyHold(GLFW_KEY_3)) {
		angularStepOX -= valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_4)) {
		angularStepOX += valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_5)) {
		angularStepOY -= valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_6)) {
		angularStepOY += valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_7)) {
		angularStepOZ += valueT * deltaTime;
	}
