...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_W)) {
		logicSpace.y += valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_S)) {
		logicSpace.y -= valueT * deltaTime;
	}
	if (window->KeyHold(GLFW_KEY_A)) {
		logicSpace.x += valueT * deltaTime;
	}
	...
	if (window->KeyHold(GLFW_KEY_Z)) {
		logicSpace.height += valueT * deltaTime;
		logicSpace.width += valueT * deltaTime;
		logicSpace.x -= valueT/2 * deltaTime;
		logicSpace.y -= valueT/2 * deltaTime;
	}
